...
class loggedUser():
    ...
    def set_user_info(self):
        if int(self.id) in boys.id.values:
            ...
            self.match_path = PATH_BOYS_SWIPE_DIR + '/{0}.log'.format(self.id)
            self.logger = setup_logger('user_logger', PATH_BOYS_SWIPE_DIR + 'first_logfile.log')


        elif int(self.id) in girls.id.values:
            self.gender = "girl"
            self.swipe_data_path  = PATH_GIRLS_SWIPE_DIR + '/{0}.csv'.format(self.id)
            self.match_path = PATH_GIRLS_SWIPE_DIR + '/{0}.log'.format(self.id)
...
def swipes_new():
    ...
    ### НАЧАЛО МАТЧИНГА
    if swipe_type == 'right':
        with open(PATH_BOYS_SWIPE_DIR + '/{0}.csv'.format(swipe_id)) as f_swipe:
            for line in csv.reader(f_swipe):
                if line == [user_id, 'right']:
                        print('MATCH!')
                        break
