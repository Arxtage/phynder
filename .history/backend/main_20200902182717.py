...
def sample_partners_v2(user_id):
    ...
    if int(user_id) in boys.id.values:
        user_swipe_data = pd.read_csv(PATH_BOYS_SWIPE_DIR + '/{0}.csv'.format(user_id))
        ...
        user_swipe_data = pd.read_csv(PATH_GIRLS_SWIPE_DIR + '/{0}.csv'.format(user_id))
        sample = boys[~boys.id.isin(user_swipe_data.id_swiped)].sample(1)

    return(sample.to_json(orient='records'), user_swipe_data)
